...
<head>
    ...
    <link rel="stylesheet" href="{{ url('js/leaflet-routing-machine/dist/leaflet-routing-machine.css') }}" />
    ...
    <script src="{{ url('js/leaflet-routing-machine/dist/leaflet-routing-machine.js') }}"></script>
</head>
...
<script>
    var map = L.map('map', {
        zoomControl: false
    }).setView([-0.025956507397552307, 109.34018363488666], 12.5);
        ...
        $.getJSON('titik/json', function(data) {
            $.each(data, function(index) {
                ...
                marker.on('mouseover', function(ev) {
                    const popupLocation = '<p>' + data[index].Title;
                    ev.target.bindPopup(popupLocation).openPopup();
                });

                marker.on('click', function(ev) {
                    const popupContent = '<h3>' + data[index].Title + '</h3>' + '<p>' +
                        data[index].Alamat + '</p>' +
    ...
    legend.onAdd = function(map) {
        ...
        div.innerHTML +=
            '<img src="images/line.jpeg" width="18px" class="icon py-2"></i><span class="px-2">Batas Kota</span><br>';
    ...
    $.getJSON('geojson/export.geojson', function(json) {
        geoLayer = L.geoJson(json, {
    ...
    function cari(id) {
        geoLayer.eachLayer(function(layer) {
            if (layer.feature.properties.id == id) {
                map.flyTo([parseFloat(layer.feature.geometry.coordinates[1]), parseFloat(layer.feature.geometry
                    .coordinates[0])], 20);
            }
        });
    }
    ...
    function showPosition(position) {
        ...
        control.spliceWaypoints(0, 1, latlang);
        map.panTo(latlang);
        map.flyTo(latlang, 15);
    };

    function keSini(Latitude, Longitude) {
        var latlang = L.latLng(Latitude, Longitude);
        control.spliceWaypoints(control.getWaypoints().length - 1, 1, latlang);
        map.zoomOut(latlang, 15);
    }
</script>
